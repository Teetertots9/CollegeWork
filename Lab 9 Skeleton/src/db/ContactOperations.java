    ...
    public void CreateContactsTable() {
        try {
            ...
            String create = "CREATE TABLE Contacts "
                    + "(cID NUMBER PRIMARY KEY, cname VARCHAR(40), address VARCHAR(30),"
                    + "pnumber VARCHAR(30), email VARCHAR(20))";
            pstmt = conn.prepareStatement(create);
            pstmt.executeUpdate();

            // Insert data into table
            String insertString1 = "INSERT INTO Contacts(cID,cname,address,pnumber,email)"
   ...
   public void createAddressBookOwnerstable() {
        ...
        try {
            String create = "CREATE TABLE AddressBookOwners "
                    + "(abID NUMBER PRIMARY KEY NOT NULL, name VARCHAR(40))";
            pstmt = conn.prepareStatement(create);
            pstmt.executeUpdate();

            // Insert data into table
            String insertString1 = "INSERT INTO AddressBookOwners(abID,name) "
    ...
    String insertString1 = "INSERT INTO ContactsAddressBook(abid,cid) "
            ...
            System.out.println("ContactsAddressBook address table created.");
     } catch (SQLException ex) {
            System.out.println("SQL Exception creating and inserting values into "
                    + "ContactsAddressBook" + ex.getMessage());
        }
}

    
    public ResultSet getOwners() {
        String sqlStatement = "SELECT * FROM AddressBookOwners order by abID";
        try {
    ...
    public ResultSet getContacts(String owner) {
        String sql1 = "SELECT * "
                + "FROM Contacts c inner join ContactAddressBook cab "
                + "on c. order by cID";
        try{
    ...
    public void deleteRecord(int id) {
        try {
            ...
            String sqlStatement = "DELETE FROM Contacts WHERE cID = ?";
